...
# build base images
buildah build --rm --force-rm --progress=plain --build-arg ORG=${ORG} --build-arg BRANCH=${BRANCH} \
  -t hysds/base:${TAG} -f docker/Dockerfile .
buildah build --rm --force-rm --progress=plain --build-arg TAG=${TAG} --build-arg ORG=${ORG} \
